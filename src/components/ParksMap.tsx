...
interface ParksMapProps {
  ...
  courtAvailability: Record<string, CourtAvailability[]>;
}

function getSlotSummary(slots: CourtAvailability[]): SlotSummary {
...
export default function ParksMap({ courts, selectedDate, onParkClick, userLocation, courtAvailability }: ParksMapProps) {
    ...
    <div className="relative w-full h-[400px] rounded-lg overflow-hidden shadow-lg">
      ...
      >
        ...
        {validCourts.map((court) => {
          const slots = courtAvailability[court.park_id] || [];
          const summary = getSlotSummary(slots);
          
          let dateStrToShow = 'All Dates';
          if (selectedDate) {
            dateStrToShow = format(selectedDate, 'MMMM d, yyyy');
          } else if (slots.length > 0) {
            // Sort by earliest date
            const earliestSlot = [...slots].sort((a, b) => a.date.localeCompare(b.date))[0];
            if (earliestSlot) {
              const [year, month, day] = earliestSlot.date.split('-');
              dateStrToShow = `Next Available: ${format(new Date(parseInt(year), parseInt(month)-1, parseInt(day)), 'MMMM d, yyyy')}`;
            }
          }
                ...
                <div className="p-3 min-w-[250px]">
                  ...
                  <div className="mt-3 space-y-1 text-sm">
                    <div className={`font-semibold ${summary.total > 0 ? 'text-green-600' : 'text-gray-600'}`}>
                      {summary.total > 0 ? (
                        `${summary.total} Available Slots - ${dateStrToShow}`
                      ) : (
                        `No available slots${selectedDate ? ` on ${dateStrToShow}` : ''}`
                      )}
                    </div>
                    {summary.total > 0 && (
